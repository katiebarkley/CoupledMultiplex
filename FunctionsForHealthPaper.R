...
# popsize, ncomms = number communities, porpbelA= prop of pop in belief A
...
pop_gen  <-function(pop_size=1000,ncomms=10,prop_belA=0.5,prop_old=0.3,prop_young=0.5, prop_child=0.2){
  ...
  child <-c(rep(1,n_Ac+n_Bc),rep(0,n_Ay+n_By+n_Ao+n_Bo))   #Indicator of child or not
  adult <-c(rep(0,n_Ac+n_Bc),rep(1,n_Ay+n_By+n_Ao+n_Bo))   #Indicator of adult or not
  old   <-c(rep(0,n_Ac+n_Bc+n_Ay+n_By),rep(1,n_Ao+n_Bo))   #Indicator if old or not
...
#For political belief B
 for(i in (pop_info$n_Ac+1):(pop_info$n_Ac+pop_info$n_Bc)){
  t_ycon     <-which(dis_mat[i,(pop_info$n_Ac+pop_info$n_Bc+pop_info$n_Ay+1):(pop_info$n_Ac+pop_info$n_Bc
                                                                              +pop_info$n_Ay+pop_info$n_By)]==1)
...
concern_setup<-function(A_concern_y,B_concern_y,A_concern_o,B_concern_o,pop_info,mat_cyo){
  ...
  for(i in 1:(pop_info$n_Ac+pop_info$n_Bc)){
    t_par       <-which(mat_cyo[1,(pop_info$n_Ac+pop_info$n_Bc+1):(pop_info$n_Ac+pop_info$n_Bc+pop_info$n_Ay+pop_info$n_By)]==1)
    initial_conc[1] <-max(initial_conc[t_par+pop_info$n_Ac+pop_info$n_Bc])
  }
  initial_conc
...
concern_timestep  <-function(pop_info,net_b,net_d,belief,concern,inf,lA_ex,lB_ex,l_conc,l_conc_o,l_inf,l_inf_o,l_hea,l_hea_o){
  ...
  conc       <-c(rep(NA,pop_info$n_Ac+pop_info$n_Bc),rbinom(pop_info$pop-pop_info$n_Ac-pop_info$n_Bc,1,
