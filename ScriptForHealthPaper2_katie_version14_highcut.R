...
outputfolder<-file.path("output14_highcut","results14_highcut")
if(!dir.exists(outputfolder)) dir.create(outputfolder,recursive = T) #if output folder doesn't exists, create it.
    
source(file.path(sourcefolder,"FunctionsForHealthPaper_katie_version - Copy.R"))
...
#params3   <-params3[,2:4]
params4   <-read.csv(file.path(sourcefolder,paramsfolder,"group_id_param14.csv"))
...
l_hea    <-params3[(nt-1)*4*77*5+(g-1)*77*5+(md-1)*5+r,3]
...
for(t in 2:time){
  ...
  if(t==2){
    ...
    dis_mat  <-network_rewire_concern(net=dis_mat,concern=start[[2]],concern.prev=rep(0,g_pop_info$g_pop),g_pop_info=g_pop_info,prop_cutg=0.25,prop_cut=0.9,
  ...
  if(t>2){
    ...
    dis_mat  <-network_rewire_concern(net=dis_mat,concern=current[[2]],
                                      ...
                                      g_pop_info=g_pop_info,prop_cut=0.9,prop_cutg=0.25,cut_to=0.001,
...
saveRDS(OUT, file.path(outputfolder,paste0("hc_nets",params1$NetSelect[nt],"mods",params2$ModSelect[md],"group",g,"rep",r,".RDS")))
...
saveRDS(OUT_G, file.path(outputfolder,paste0("hc_group_nets",params1$NetSelect[nt],"mods",params2$ModSelect[md],"group",g,"rep",r,".RDS")))
