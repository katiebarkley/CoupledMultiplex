...
outputfolder<-file.path("output_ng","results4_ng")
...
for(t in 2:time){
  ...
  if(t==2){
    ...
    dis_mat  <-network_rewire_concern(net=dis_mat,concern=start[[2]],concern.prev=rep(0,pop_info$pop),pop_info=pop_info,prop_cut=0.5,cut_to=0.001)
  ...
  if(t>2){
    ...
    dis_mat  <-network_rewire_concern(net=dis_mat,concern=current[[2]],
                                      ...
                                      pop_info=pop_info,prop_cut=0.5,cut_to=0.001)
    dis_mat  <-network_rewire_infectionS(net=dis_mat,status=statuses[[t-1]],
                                       pop_info=pop_info,cut_to=0.001)
    #dis_mat<-network_rewire_infectionM(net=dis_mat,status=statuses[[t-1]],
    #                                   pop_info=pop_info,cut_to=0.001)
...
saveRDS(OUT, file.path(outputfolder,paste0("ng_nets",params1$NetSelect[nt],"mods",params2$ModSelect[md],"rep",r,".RDS")))
