...
outputfolder<-file.path("output_ng_highcut","results4_ng_highcut")
...
S_E     <- 0.3/mean(colSums(dis_mat)) #Probability of becoming exposed having contacted an infectious                                              individual (daily)
E_I1    <-5.1     # lambda for a Poisson draw for the length of this period
...
yI1_R   <-6.7     #lambda for a Poisson draw for duration of a pre-symptomatic/mild                                             infection - this is now misnamed as impossible to recover - simply transition to I2.
oI1_R   <-6.7     #and same for old
yI2_R   <-10      #lambda for a Poisson draw for duration of a serious infection
oI2_R   <-10      #and same for old
...
for(t in 2:time){
  ...
  if(t==2){
    ...
    dis_mat  <-network_rewire_concern(net=dis_mat,concern=start[[2]],concern.prev=rep(0,pop_info$pop),pop_info=pop_info,prop_cut=0.9,cut_to=0.001)
    ...
    current  <-concern_timestep(pop_info=pop_info,net_b=info_mat,net_d=dis_mat,
                                 belief=start[[1]],concern=start[[2]],inf=inf,lA_ex,
                                 lB_ex,l_conc,l_conc_o,l_inf,l_inf_o,l_hea,l_hea_o)
    
    belief[[2]]  <-current[[1]]
    concern[[2]] <-current[[2]]
    
    dis  <-infection_timestep(pop_info=pop_info,status=statuses[[t-1]],net=dis_mat,
  ...
  if(t>2){
    ...
    dis_mat  <-network_rewire_concern(net=dis_mat,concern=current[[2]],
                                      ...
                                      pop_info=pop_info,prop_cut=0.9,cut_to=0.001)
    dis_mat  <-network_rewire_infectionS(net=dis_mat,status=statuses[[t-1]],
                                       pop_info=pop_info,cut_to=0.001)
    #dis_mat<-network_rewire_infectionM(net=dis_mat,status=statuses[[t-1]],
    #                                   pop_info=pop_info,cut_to=0.001)
    ...
    current  <-concern_timestep(pop_info=pop_info,net_b=info_mat,net_d=dis_mat,
                                 ...
                                 lA_ex,lB_ex,l_conc,l_conc_o,l_inf,l_inf_o,l_hea,
                                 l_hea_o)
    
    belief[[t]]  <-current[[1]]
    concern[[t]] <-current[[2]]  
    
    dis       <-infection_timestep(pop_info=pop_info,status=statuses[[t-1]],
...
saveRDS(OUT, file.path(outputfolder,paste0("ng_hc_nets",params1$NetSelect[nt],"mods",params2$ModSelect[md],"rep",r,".RDS")))
